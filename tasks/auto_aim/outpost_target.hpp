 ...
 * 坐标系说明（重要！）
 * ============================================================================
 * 本模块中的 (cx, cy) 是"云台到前哨站中心的相对位置向量"，用重力对齐坐标表达。
 * 这不是场地固定坐标！当车体/云台移动时，(cx, cy) 会变化。
 *
 * 设计哲学：
 * - 采用"强先验 + 慢漂移"模型：小过程噪声 (v1=0.01) 抑制快速跳变
 * - 允许 EKF 缓慢更新 (cx, cy)，适应车体平移
 * - omega 独立估计，不放入 EKF
 *
 * 如果打不中，先检查：
 * 1. timing：相机时间戳 vs IMU 时间戳是否同步？
 * 2. 标定：R_camera2gimbal, t_camera2gimbal 是否准确？
 * 3. 姿态：IMU 四元数是否正确补偿云台转动？
 * ============================================================================
 *
 * 核心思想：
 * - 将三层装甲板的运动压缩成旋转的一层
...
private:
  ...
  double omega_max_abs_ = 3.0;  // 降低限幅：减少极端值影响
  ...
  double xy_residual_gate_m_ = 0.40;          // 40cm（放宽：PnP误差+三层透视差异）
